...
from cojoden_functions import color_graph_background, convert_df_string_to_search_string
...
def extract_villes(df, dest_path, dest_file_name='villes_departement_region_pays.csv',index=True, verbose=0):
    ...
    df_cities = df_cities.rename(columns={'geo_ville':'ville', 'geo_departement':'departement', 'geo_region':'region1','geo_pays_region_ville':'pays'})
...
def extract_musees(df, dest_path, dest_file_name='musees.csv', verbose=0):
    df_musee = df[['museo','nom_officiel_musee','pop_coordonnees','geo_ville', 'latitude', 'longitude']]
    if verbose > 0:
...
def extract_oeuvres(df, dest_path, dest_file_name='oeuvres.csv', verbose=0):
    ...
    df1.loc[df1['creation_millesime'].notna(),'millesime_clean'] =  df1.loc[df1['creation_millesime'].notna(),'creation_millesime'].apply(lambda x: standardize_creation_millesime(str_millesime=x, verbose=verbose))
    df1.loc[df1["millesime_clean"].notna(),"annee_debut"] = df1.loc[df1["millesime_clean"].notna(),"millesime_clean"].apply(lambda x: x[0])
    df1.loc[df1["millesime_clean"].notna(),"annee_fin"] = df1.loc[df1["millesime_clean"].notna(),"millesime_clean"].apply(lambda x: x[1])
    
    df1.loc[df1["creation_lieux"].notna(),"creation_lieux"] = df1.loc[df1["creation_lieux"].notna(),"creation_lieux"].apply(lambda x: _clean_lieux_creation(x, verbose=verbose))
...
def clean_metier(input_str, verbose=0):
    ...
    if res is not None and isinstance(res, str):
        if len(res)<4:
            ...
            "pastiche","République", "suiveur", "MULLER"
            ]
